...
def generate_prediction(df, num_predictions=5, seed_source="", actual_draw=None):
    ...
    _, closest_cluster_numbers_raw = find_closest_cluster(kmeans, recent_draws)
    closest_cluster_numbers = list(set(closest_cluster_numbers_raw.astype(int)))
    ...
    for _ in range(num_predictions):
        ...
        while attempt < 100:
            ...
            joker_weights = [joker_counter.get(j, 1) for j in joker_candidates]
            selected_joker = random.choices(joker_candidates, weights=joker_weights, k=1)[0]
